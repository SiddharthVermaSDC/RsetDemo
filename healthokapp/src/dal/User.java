...
public class User {
	...
	public StatusCode fullRegister(model.UserFull us) {
		...
		try {
			...
			if (us.getUserPhoneNumber() != null) {
				String phonenumber = new String(
						"insert into Useradditionalphonenumber(UserId,AdditionalPhoneNumber1,AdditionalPhoneNumber2,AdditionalPhoneNumber3)"
								+ "values(?,?,?,?);");
				for (model.UserPhoneNumber userphonenumber : us.getUserPhoneNumber()) {
					ps = connection.prepareStatement(phonenumber);
					ps.setInt(1, userid);
					ps.setString(2, userphonenumber.getAdditionalPhoneNumber1());
					ps.setString(3, userphonenumber.getAdditionalPhoneNumber2());
					ps.setString(4, userphonenumber.getAdditionalPhoneNumber3());
					ps.executeUpdate();
				}
			}
	...
	public StatusCode familyRegister(model.UserFull us, int userid) {
		...
		try {
			...
			queryToInsert = new String("insert into memberdetails(FirstName,LastName,Sex,DOB,BloodGroup,"
					+ "Allergies,CurrentMedications,Diabetic,BP,HeartProblems,RecurringTests,"
					+ "LongTermCareNeeds,Comments,UserId)" + " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
	...
	public model.User getUser(int userId) {
		...
		try {
			...
			while (rs.next()) {
				...
				user = new model.User(); // since this is query by PK will get
											// only one row.
	...
	public int loginCheck(GetSetLogin gs) {
		...
		try {
			...
			connection = Database.createConnection();
			String query = "select UserId from User where (EmailId=\"" + gs.getLoginId() + "\" or Mobile=\""
					+ gs.getLoginId() + "\") and Password=\"" + gs.getPassword() + "\";";
	...
	public ArrayList<model.UserFull> getAllUsers() {
		...
		String queryUser = "select * from User limit 5";
		String queryMemberDetail = "select * from MemberDetails where userid = ?";
